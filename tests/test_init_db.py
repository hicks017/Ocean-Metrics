...
class TestInitDB(unittest.TestCase):
    ...
    @patch("src.storage.load_ddl")
    @patch("src.storage.USE_POSTGRES", new_callable=lambda: False)
    def test_create_table_failure_sqlite(self, mock_use_postgres, mock_load_ddl):
        mock_conn = MagicMock()
        mock_cursor = mock_conn.cursor.return_value
        mock_load_ddl.side_effect = lambda table: f"CREATE TABLE {table} (id {{id_def}});"
        mock_cursor.execute.side_effect = [
    ...
    @patch("src.storage.load_ddl")
    @patch("src.storage.USE_POSTGRES", new_callable=lambda: False)
    def test_create_index_failure_sqlite(self, mock_use_postgres, mock_load_ddl):
        mock_conn = MagicMock()
        mock_cursor = mock_conn.cursor.return_value
        mock_load_ddl.side_effect = lambda table: f"CREATE TABLE {table} (id {{id_def}});"
        mock_cursor.execute.side_effect = [
    ...
    @patch("src.storage.load_ddl")
    @patch("src.storage.USE_POSTGRES", new_callable=lambda: False)
    def test_create_table_before_index_sqlite(self, mock_use_postgres, mock_load_ddl):
        mock_conn = MagicMock()
        mock_cursor = mock_conn.cursor.return_value
        mock_load_ddl.side_effect = lambda table: f"CREATE TABLE {table} (id {{id_def}});"
        ...
        # Assert CREATE TABLE appears before CREATE INDEX for each table
        for table in ["wind", "swell", "temps", "energy"]:
            create_table_call = call(f"CREATE TABLE {table} (id INTEGER PRIMARY KEY AUTOINCREMENT);")
            create_index_calls = [
                call(f"CREATE INDEX IF NOT EXISTS idx_{table}_date_utc ON {table}(Date_utc);"),
                call(f"CREATE INDEX IF NOT EXISTS idx_{table}_station ON {table}(station);")
            ]
